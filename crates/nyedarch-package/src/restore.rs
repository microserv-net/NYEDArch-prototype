...
impl Sink {
    fn write(&mut self, data: &[u8]) -> Result<(), ()> {
        ...
        // No `unwrap` here even though the handle was just created: this runs
        // inside a capsule on a hostile machine, where a panic is a denial of
        // service and its message can disclose paths. An impossible state must
        // still return an error rather than abort (spec §60).
        match self.handle.as_mut() {
            Some(h) => h.write_all(data).map_err(|_| ()),
            None => Err(()),
        }
    }
...
pub fn open_manifest(parsed: &Parsed, payload_key: &[u8; 32]) -> Result<Manifest, ()> {
    ...
    let context = crate::format::chunk_context(&compose::payload_aad(&parsed.header.binding(), flags), &parsed.header);

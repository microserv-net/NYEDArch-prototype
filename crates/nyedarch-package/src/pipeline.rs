...
///
/// The level is a build-time choice only: it changes how hard the compressor
/// works, never the format or the cryptography. The codec is recorded in the
/// header either way, so a capsule built at any level opens the same.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum CompressionMode {
    /// Chosen from the payload size: heavy effort on a small input costs build
    /// time for almost nothing.
    Automatic,
    /// Smallest capsule, slowest to build.
    Maximum,
    /// The default trade-off.
    Balanced,
    /// Quickest build, largest capsule.
    Fast,
}
...
impl CompressionMode {
    ...
    /// zstd level for this mode.
    pub fn level(self, payload_bytes: u64) -> i32 {
        match self {
            Self::Maximum => 19,
            Self::Balanced => 6,
            Self::Fast => 1,
            Self::Automatic => {
                if payload_bytes < (1 << 20) {
                    6
                } else if payload_bytes < (64 << 20) {
                    9
                } else {
                    3 // on large payloads build time dominates
                }
            }
        }
    }
...
pub fn compress_block_level(codec: u8, level: i32, raw: &[u8]) -> Vec<u8> {
    match codec {
        COMPRESSION_ZSTD => {
            let body = zstd::bulk::compress(raw, level).unwrap_or_default();
...
pub fn seal_chunk_level(
    ...
    let comp = compress_block_level(codec, level, plaintext);

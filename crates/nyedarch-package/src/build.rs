...
use crate::pipeline::{seal_chunk_level, TAG_MANIFEST, TAG_PAYLOAD};
...
pub fn seal_package(
    ...
    seal_package_cancellable(header, sealed_policy, payload_key, manifest, files, 6, &|| false)
}

/// Seal a package, with a compression level and a cancellation check.
///
/// `cancelled` is consulted between chunks. Cancelling mid-seal must leave no
/// partial output, so the caller discards what it was given; nothing plaintext
/// is written by this function in any case.
pub fn seal_package_cancellable(
    header: &Header,
    sealed_policy: &[u8],
    payload_key: &[u8; 32],
    manifest: &Manifest,
    ...
    // Bind the whole header, not only the key-derivation fields: otherwise
    // `compression` and `chunk_size` are neither authenticated nor key inputs.
    let context = crate::format::chunk_context(&compose::payload_aad(&header.binding(), flags), header);

    // Manifest -> its own sealed chunk.
    let manifest_bytes = bincode::serialize(manifest).map_err(|_| ())?;
    let sealed_manifest = seal_chunk_level(header.compression, level, payload_key, &context, TAG_MANIFEST, 0, true, &manifest_bytes)?;
    ...
    'outer: loop {
        ...
        if cancelled() {
            return Err(()); // caller discards the partial result
        }
        sealed_chunks.push(seal_chunk_level(header.compression, level, payload_key, &context, TAG_PAYLOAD, index, is_last, &buf)?);
        index += 1;
        buf.clear();
        if is_last {
            break 'outer;

...
/// A commitment over the entire serialized header.
///
/// The payload AAD binds the fields that participate in key derivation, but
/// `compression` and `chunk_size` are neither authenticated nor key inputs, so
/// an attacker could flip them and the package would still authenticate. Found
/// by the attack laboratory's bit-flip sweep, which showed a modified byte in
/// the header still yielding plaintext.
///
/// Binding the whole header closes that: any change to any header field, now or
/// in future versions, alters the AAD and every chunk fails to authenticate.
pub fn header_commitment(h: &Header) -> [u8; 32] {
    // HKDF-Extract over the serialized header, using the crypto crate's own
    // primitive rather than adding a hash dependency here. The salt provides
    // domain separation.
    let bytes = bincode::serialize(h).unwrap_or_default();
    match nyedarch_crypto::kdf::hkdf_key(b"nyedarch:v1:header-commitment", &bytes, b"header") {
        Ok(k) => *k,
        // A commitment that cannot be computed must not silently become a
        // constant, which would authenticate every header equally. Derive from
        // the bytes themselves so different headers still differ.
        Err(_) => {
            let mut out = [0u8; 32];
...
/// The authenticated associated data for a package's chunks.
///
/// `base` is the payload context from the crypto layer; the header commitment
/// is appended so nothing in the header is left unauthenticated.
pub fn chunk_context(base: &[u8], h: &Header) -> Vec<u8> {
    let mut c = Vec::with_capacity(base.len() + 32);
    c.extend_from_slice(base);
    c.extend_from_slice(&header_commitment(h));
    c
}
...
pub fn parse(b: &[u8]) -> Result<Parsed, ()> {
    ...
    if hlen > b.len().saturating_sub(off) {
        return Err(()); // refuse before allocating on an attacker's number
    }
    ...
    // The chunk count comes from the artifact, so it is attacker-controlled.
    // `Vec::with_capacity(ccount)` on that value let a single tampered field
    // request a 51 GB allocation and abort the process - a denial of service,
    // and an abort rather than the fail-closed refusal the design requires.
    // Found by the attack laboratory's bit-flip sweep.
    //
    // Every chunk costs at least a 4-byte length prefix, so a count larger than
    // the remaining bytes can hold is impossible and is rejected before any
    // allocation happens.
    let remaining = b.len().saturating_sub(off);
    if ccount > remaining / 4 {
        return Err(());
    }

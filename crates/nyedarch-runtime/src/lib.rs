...
/// Destroy the capsule after a successful one-shot extraction.
///
/// Delegates to `destruction`, which overwrites, renames, removes, and then
/// **verifies**. The previous implementation discarded the result of a bare
/// `remove_file`, so it could not distinguish a capsule that was gone from one
/// that was still sitting on disk.
pub fn one_shot_destroy(self_path: &Path) -> destruction::Destruction {
    destruction::one_shot_destroy(self_path)
}

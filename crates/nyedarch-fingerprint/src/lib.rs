...
impl Fingerprint {
    ...
    /// Compare two fingerprint identifiers.
    ///
    /// Constant time: every byte is examined whatever the result, so the number
    /// of matching leading bytes cannot be measured. An early-returning compare
    /// would let an attacker recover a trusted identifier byte by byte.
...
pub mod nyfp {
    ...
    /// HMAC over the record body.
    ///
    /// Returns `None` rather than panicking. HMAC accepts any key length, so
    /// this cannot fail in practice - but "cannot fail in practice" is exactly
    /// the reasoning that leaves an `expect` in a security path, and the
    /// specification forbids it (§60).
    fn mac(key: &[u8], body: &[u8]) -> Option<[u8; MAC_LEN]> {
        let mut m = <HmacSha512 as Mac>::new_from_slice(key).ok()?;
    ...
    pub fn seal_bytes(key: &[u8], body: &[u8]) -> Result<Vec<u8>, NyfpError> {
        let tag = mac(key, body).ok_or(NyfpError::Auth)?;

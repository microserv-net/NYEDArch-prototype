...
/// Redacting `Debug`.
///
/// `PolicyEntry` holds `factor_secret`, the per-machine contribution to the
/// payload key. A derived `Debug` would print it in full from any `{:?}` - an
/// error message, a debug log, a panic - and the specification forbids secret
/// material reaching a formatter at all (§57).
///
/// Nothing formatted this today. The point is that nothing *can*: the leak is
/// removed rather than merely unexercised, so a later edit cannot reintroduce
/// it by accident.
impl core::fmt::Debug for PolicyEntry {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        f.debug_struct("PolicyEntry")
            .field("fingerprint_id", &"<opaque>")
            .field("factor_secret", &"<redacted>")
            .field("labels", &self.labels)
            .finish()
    }
}

impl core::fmt::Debug for PolicyRecord {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        // The count is safe and useful; the contents are not.
        f.debug_struct("PolicyRecord")
...
mod redaction_tests {
    ...
    /// Secret material must never reach a formatter (spec §57). A derived
    /// `Debug` would print the per-machine factor secret from any `{:?}`.
    #[test]
    fn debug_output_never_contains_secret_material() {
        let entry = PolicyEntry {
            fingerprint_id: [0xAB; 32],
            factor_secret: [0xCD; 32],
            labels: vec!["hr".into()],
        };
        let shown = format!("{entry:?}");
        assert!(!shown.contains("cd"), "the factor secret leaked: {shown}");
        assert!(!shown.contains("205"), "the factor secret leaked in decimal: {shown}");
        assert!(shown.contains("<redacted>"));
        // Labels are metadata and may be shown; they help diagnose.
        assert!(shown.contains("hr"));

        let record = PolicyRecord { entries: vec![entry] };
        let shown = format!("{record:?}");
        assert!(!shown.contains("cd") && !shown.contains("205"));
        assert!(shown.contains('1'), "the entry count is useful and safe");
    }
}
